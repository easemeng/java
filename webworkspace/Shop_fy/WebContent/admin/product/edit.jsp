...
<HTML>
	...
	<body>
		<!--  -->
		<form id="userAction_save_do" name="Form1" action="${pageContext.request.contextPath}/admin/product/AdminProductServlet?state=update&pid=${product.pid}&pimage=${product.pimage}" method="post" enctype="multipart/form-data">
			...
			<table cellSpacing="1" cellPadding="5" width="100%" align="center" bgColor="#eeeeee" style="border: 1px solid #8ba7e3" border="0">
				...
				<tr>
					...
					<td class="ta_01" bgColor="#ffffff">
						...
						<select name="is_hot">
						<option value="1"
							<c:if test="${product.is_hot==1}">selected</c:if>>是</option>
						<option value="0"
							<c:if test="${product.is_hot==0}">selected</c:if>>否</option>
					...
					<td class="ta_01" bgColor="#ffffff" colspan="3">
						<select name="cid">
							<c:forEach items="${categoryList}" var="category">
								<option value="${category.cid}" <c:if test="${category.cid==product.cid}">selected</c:if> >${category.cname }</option>
							</c:forEach>
